...
jQuery(document).ready(function ($) {
  ...
  $('#photo_thumbs a').click(function(e){
	...
	$('ul.navlinks').each(function () {
        ...
        // Deprecated Mike 5.19.12 - in favor of auto env. recognition
...
//	});
    // Added Mike 5.19.12
    // looks at current url and compares the carousel items to search for the right active state
    var lastPath = window.location.pathname.split( '/' ).pop().replace(/.php/, "");
    if(lastPath.length > 0) {
        $('#mainNav li a').each(function(index) {
            if($(this).attr('href') == lastPath) {
